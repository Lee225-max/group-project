...
class DatabaseManager:
    ...
    def get_user_knowledge_items(self, user_id):
        return self.get_knowledge_with_review_status(user_id)

    def cancel_review_schedule(self, knowledge_item_id: int, user_id: int) -> bool:
        """取消知识点的所有未完成复习计划"""
        session = self.get_session()
        try:
            print(f"🔍 [CANCEL DEBUG] 开始取消知识点 {knowledge_item_id} 的复习计划 - manager.py")
            ...
            print(f"🗑️ 找到 {len(pending_schedules)} 个未完成计划")

            for schedule in pending_schedules:
                session.delete(schedule)
                print(f"🗑️ 删除复习计划ID: {schedule.id}")

            session.commit()
            print(f"✅ 已取消知识点 {knowledge_item_id} 的复习计划")
            return True
    ...
    def update_review_schedule_time(self, schedule_id: int, new_time) -> bool:
        """更新复习计划的下次复习时间"""
        session = self.get_session()
        try:
            schedule = session.query(ReviewSchedule).filter(
            ...
            if not schedule:
                print(f"❌ [UPDATE DEBUG] 未找到复习计划: {schedule_id}")
                session.close()
                return False
            ...
            schedule.scheduled_date = new_time
            session.commit()
            print(f"✅ [UPDATE DEBUG] 已更新复习计划 {schedule_id} 的时间为 {new_time}")
            return True

        except Exception as e:
            session.rollback()
            print(f"❌ [UPDATE ERROR] 更新复习计划时间失败: {e}")
            return False
        finally:
            session.close()