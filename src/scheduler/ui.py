    ...
    def load_knowledge_item(self):
        ...
        try:
            ...
            if not self.knowledge_item:
                print(f"❌ 找不到知识点: ID {knowledge_item_id} - ui.py:87")
        except Exception as e:
            print(f"❌ 加载知识点失败: {e} - ui.py:89")
        finally:
            session.close()
...
class ReviewSchedulerFrame(ctk.CTkFrame):
    ...
    def __init__(self, parent, current_user, db_manager):
        ...
        print(f"🎯 今日复习界面初始化完成  用户ID: {self.current_user.id} - ui.py:258")

        self.create_widgets()
        print("🎯 今日复习界面组件创建完成 - ui.py:261")

        self.load_today_reviews()
        print("🎯 今日复习界面数据加载完成 - ui.py:264")
    ...
    def create_widgets(self):
        ...
        print(f"📦 内容框架创建: {content_frame.winfo_exists()} - ui.py:294")
        ...
        print(f"📦 滚动框架创建: {self.review_list_frame.winfo_exists()} - ui.py:302")
    ...
    def clear_widgets(self):
        ...
        try:
            ...
            print(f"清除组件时出错: {e} - ui.py:330")

    def load_today_reviews(self):
        """加载今日复习计划"""
        print("🔄 今日复习界面开始加载数据 - ui.py:334")
        # 安全地清除现有内容
        self.clear_widgets()

        try:
            print(f"🔍 调用调度器服务获取今日复习计划，用户ID: {self.current_user.id} - ui.py:339")
            ...
            if hasattr(self.scheduler_service, 'get_today_review_plans'):
                ...
                print("✅ 使用 get_today_review_plans 方法 - ui.py:348")
            # 方法2: 尝试 get_today_reviews
                ...
                today_reviews = self.scheduler_service.get_today_reviews(
                    self.current_user.id)
                print("✅ 使用 get_today_reviews 方法 - ui.py:353")
            else:
                print("❌ 调度器服务中没有找到获取今日复习计划的方法 - ui.py:355")
                today_reviews = []

            print(f"📊 今日复习界面收到 {len(today_reviews)} 个复习计划 - ui.py:358")

            # 调试：打印接收到的数据
            for i, review in enumerate(today_reviews):
                print(f"📋 复习计划 {i + 1}: {type(review)} - ui.py:362")
                if hasattr(review, '__dict__'):
                    print(f"属性: {review.__dict__} - ui.py:364")
                elif isinstance(review, dict):
                    print(f"数据: {review} - ui.py:366")
                else:
                    print(f"值: {review} - ui.py:368")
            ...
            print(f"✅ 成功创建 {len(self.current_widgets)} 个复习项目 - ui.py:394")

        except Exception as e:
            print(f"❌ 加载复习计划失败: {str(e)} - ui.py:397")
    ...
    def create_review_item(self, review):
        """创建复习项目UI"""
        print(f"🔧 创建复习项目: {type(review)} - ui.py:411")

        try:
            # 确保review是字典格式
            review = self._ensure_dict_format(review)
            print(f"🔧 转换后格式: {type(review)} - ui.py:416")
            ...
            try:
                ...
                if not knowledge_item:
                    print(f"❌ 找不到知识点: ID {knowledge_item_id} - ui.py:436")
                    return None
                ...
                print(f"✅ 成功创建复习项目: {knowledge_item.title} - ui.py:543")
                return item_frame

            except Exception as e:
                print(f"❌ 创建复习项目时出错: {e} - ui.py:547")
                return None
            finally:
                session.close()

        except Exception as e:
            print(f"❌ 处理复习项目时出错: {e} - ui.py:553")
            return None
