...
class KnowledgeManagementFrame(ctk.CTkFrame):
    ...
    def add_knowledge_item(self):
        """添加知识点"""
        KnowledgeItemDialog(self, self.current_user, self.knowledge_service, self.load_knowledge_items)
    
    def edit_item(self, item):
        """编辑知识点"""
        KnowledgeItemDialog(self, self.current_user, self.knowledge_service, self.load_knowledge_items, item)
    ...
    def on_search(self, event=None):
        ...
        if search_term:
            items = self.knowledge_service.search_knowledge_items(self.current_user.id, search_term)
...
class KnowledgeItemDetailDialog(ctk.CTkToplevel):
    ...
    def create_widgets(self):
        ...
        # 内容
        content_label = ctk.CTkLabel(main_container, text="内容:", font=ctk.CTkFont(size=14, weight="bold"))
